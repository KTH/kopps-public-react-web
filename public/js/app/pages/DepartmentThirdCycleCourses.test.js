...
describe('Render component DepartmentCourses and check its menu, content and links for third cycle studies', () => {
  test('check all links on the page in English', () => {
    ...
    // Main content links
    expect(links[1]).toHaveTextContent('FAF3115')
    expect(links[1].href).toStrictEqual('http://localhost/student/kurser/kurs/FAF3115?l=en')
    expect(links[2]).toHaveTextContent('Concrete and Other Cement Based Materials')
    expect(links[2].href).toStrictEqual('http://localhost/student/kurser/kurs/FAF3115?l=en')

    expect(links[3]).toHaveTextContent('F1C5031')
    expect(links[3].href).toStrictEqual('http://localhost/student/kurser/kurs/F1C5031?l=en')
    expect(links[4]).toHaveTextContent('Design Methods for Reinforced Concrete Structures')
    expect(links[4].href).toStrictEqual('http://localhost/student/kurser/kurs/F1C5031?l=en')
    // Footer links
  ...
  test('check all links on the page in Swedish', () => {
    ...
    // Main content links
    expect(links[1]).toHaveTextContent('FAF3115')
    expect(links[1].href).toStrictEqual('http://localhost/student/kurser/kurs/FAF3115')
    expect(links[2]).toHaveTextContent('Betong och andra cementbaserade material')
    expect(links[2].href).toStrictEqual('http://localhost/student/kurser/kurs/FAF3115')

    expect(links[3]).toHaveTextContent('F1C5031')
    expect(links[3].href).toStrictEqual('http://localhost/student/kurser/kurs/F1C5031')
    expect(links[4]).toHaveTextContent('Dimensioneringsmetoder för armerade betongkonstruktioner')
    expect(links[4].href).toStrictEqual('http://localhost/student/kurser/kurs/F1C5031')
    // Footer links
